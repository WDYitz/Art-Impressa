...
<head>
  ...
  <!-- ── Primary SEO ─────────────────────────────────────────────────── -->
  <title><?= esc(SITE_NAME) ?> | Gráfica e Editora em São Paulo – Impressão Offset e Digital</title>
  ...
  <script>
    tailwind.config = {
      theme: {
        extend: {
          colors: {
            gold:     { DEFAULT: '#C9973B', light: '#E8C57D' },
            ink:      '#0D0D0D',
            cream:    '#F7F4EF',
            stone:    '#E8E0D5',
            muted:    '#717182',
  ...
  <style>
    ...
    .reveal-left  { opacity:0; transform:translateX(-64px); transition:opacity .9s  ease, transform .9s  ease; }
    .reveal-right { opacity:0; transform:translateX(64px);  transition:opacity .9s  ease, transform .9s  ease; }
    ...
    @keyframes g-cw  { to{ transform:rotate(360deg);  } }
    @keyframes g-ccw { to{ transform:rotate(-360deg); } }
    .gear-cw   { animation: g-cw  20s linear infinite; }
    .gear-ccw  { animation: g-ccw 15s linear infinite; }
    .gear-cw2  { animation: g-cw  32s linear infinite; }
    .gear-ccw2 { animation: g-ccw 10s linear infinite; }

    /* ── Ink-drop float animation ─────────────────────────── */
    @keyframes ink-float {
      0%,100%{ transform:translateY(0)    scale(1);   opacity:.10; }
      50%    { transform:translateY(-22px) scale(1.3); opacity:.40; }
    }
    .ink { animation: ink-float var(--dur,4s) ease-in-out var(--del,0s) infinite; }
    ...
    .qword    { opacity:0; display:inline-block; }
    .qword.on { animation: word-rise .6s cubic-bezier(.22,1,.36,1) forwards; }

    /* ── Press-roller pulse ───────────────────────────────── */
    @keyframes roller { 0%,100%{transform:scale(1);opacity:.3} 50%{transform:scale(1.5);opacity:1} }
    .roller { animation: roller 1.5s ease-in-out infinite; }

    /* ── Scroll-down bounce ───────────────────────────────── */
    @keyframes bdwn { 0%,100%{transform:translateY(0)} 50%{transform:translateY(8px)} }
    .bounce-d { animation: bdwn 2s ease-in-out infinite; }

    /* ── Hero crossfade slides ────────────────────────────── */
    .slide { position:absolute; inset:0; transition:opacity .9s ease; }

    /* ── Product/reason card bottom accent line ───────────── */
    .card-accent-line {
      position:absolute; bottom:0; left:0;
      height:2px; width:0; background:#C9973B;
    ...
    .nav-ul::after {
      ...
      background:#C9973B; transition:width .3s ease;
    }
    .nav-ul:hover::after { width:100%; }

    /* ── Navbar scroll state ──────────────────────────────── */
    .nav-solid       { background-color:rgba(13,13,13,.97) !important; box-shadow:0 2px 24px rgba(0,0,0,.3); }
    ...
    "@graph": [
      {
        ...
        "aggregateRating": { "@type": "AggregateRating", "ratingValue": "4.9", "bestRating": "5", "ratingCount": "312" }
      },
      {
        "@type": "WebPage",
        ...
        "breadcrumb": {
          "@type": "BreadcrumbList",
          "itemListElement": [
            { "@type": "ListItem", "position": 1, "name": "Início",   "item": "<?= esc(SITE_URL) ?>" },
            { "@type": "ListItem", "position": 2, "name": "Sobre Nós","item": "<?= esc(SITE_URL) ?>/#about" },
            { "@type": "ListItem", "position": 3, "name": "Produtos", "item": "<?= esc(SITE_URL) ?>/#products" },
            { "@type": "ListItem", "position": 4, "name": "Contato",  "item": "<?= esc(SITE_URL) ?>/#contact" }
          ]
        }
      }
    ]
